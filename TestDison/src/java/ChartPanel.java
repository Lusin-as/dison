...
public class ChartPanel extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        synchronized (parent.parent) {
            ...
            int count = parent.tableData.getRowCount();
            for (int i = 0; i < count; i++) {

                clipped.setColor(parent.data.getMoteByOrder(i).getColor());
                drawGraph(clipped, parent.data.getMoteByOrder(i).getMoteId());
            }
        }
    }
    ...
    boolean setYAxis(int newy0, int newy1) {
        if (newy0 >= newy1 || newy0 < 0 || newy0 > 5000
                || newy1 < 0 || newy1 > 5000) {
            return false;
        }
        gy0 = newy0;
